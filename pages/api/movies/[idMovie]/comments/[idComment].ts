...
export default async function handler(
    ...
    if (!ObjectId.isValid(idMovie as string) || !ObjectId.isValid(idComment)) {
        return res.status(400).json({status: 400, message: 'Invalid movie or comment ID'});
    }
    ...
    switch (req.method) {
        case 'GET':
            try {
                ...
                return res.status(200).json({status: 200, data: comment, message: 'Comment retrieved successfully'});
            } catch (error) {
                console.error("Failed to retrieve comment:", error);
                return res.status(500).json({status: 500, message: "Failed to retrieve comment"});
            }

        case 'PUT':
            try {
                if (!req.body) {
                    return res.status(400).json({status: 400, message: 'Comment data is required'});
                }
                const commentUpdate = req.body as Partial<IComment>;
                const updatedComment = await CommentService.updateComment(idComment as string, commentUpdate);
                if (!updatedComment) {
                    return res.status(404).json({status: 404, message: 'Comment not found'});
                }
                ...
                return res.status(500).json({status: 500, message: "Failed to update comment"});
            }
        ...
        case 'DELETE':
            try {
                ...
                return res.status(500).json({status: 500, message: "Failed to delete comment"});
            }
